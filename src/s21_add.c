...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (check_bit3(value_1) || check_bit3(value_2) || result == NULL)
    code = AR_INPUT;
  if (code == AR_OK && is_zero(value_2)) {
    *result = value_1;
  } else if (code == AR_OK && is_zero(value_1)) {
    *result = value_2;
  } else if (code == AR_OK) {
...
int s21_add_big(s21_big_decimal num_1, s21_big_decimal num_2,
  ...
  int mem = 0;
  for (int i = 0; i < 224; i++) {
    int res = get_bit_big(num_1, i) + get_bit_big(num_2, i) + mem;
    mem = res / 2;
    set_bit_big(result, i, res % 2);
  }

  if (mem == 1 && get_sign_big(num_1) == PLUS)
    code = AR_BIG;
  else if (mem == 1 && get_sign_big(num_1) == MINUS)
    code = AR_SMALL;
  set_scale_big(result, get_scale_big(num_1));
  set_sign_big(result, get_sign_big(num_1));
